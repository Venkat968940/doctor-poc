...
const PaymentSuccess = ({slot, data}) => {
  ...
  return (
    ...
    <Grid2 sx={{display:"flex"}}>
        ...
        <Typography variant="body1" fontWeight={600} marginRight={1} >{data.doctor_name}</Typography>
    ...
    <Box sx={{display:"flex", alignItems:"center"}}>
        ...
        <Typography variant="body1"  marginLeft={1} >{slot?.date}</Typography>
        </Box>
        <Box sx={{display:"flex", alignItems:"center"}}>
            <Schedule  color="primary" sx={{fontSize:22}} /> 
        <Typography variant="body1" marginLeft={1} >{slot?.slot_info?.time}</Typography>
        </Box>
